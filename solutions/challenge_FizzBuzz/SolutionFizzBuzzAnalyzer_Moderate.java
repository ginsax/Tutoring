...
public class SolutionFizzBuzzAnalyzer_Moderate {
	...
	/** The fizz value. */
	private final int	mFizz;
	/** The buzz value. */
	private final int	mBuzz;
	...
	/**
	 * Constructor of the moderate Fizz Buzz Analyzer. Allows for variable values
	 * for both the 'Fizz' and 'Buzz' fields.
	 * 
	 * @param fizz
	 *          First of two variables, is generally the smaller value.
	 * @param buzz
	 *          Second of two variables, is usually the larger value.
	 */
	public SolutionFizzBuzzAnalyzer_Moderate(final int fizz, final int buzz) {
		mFizz = fizz;
		mBuzz = buzz;
	}
	 ...
	 *          The number that is to be analyzed.
	...
	public FizzBuzzAnalysisResult analyze(final int numberToAnalyze) {
		if (numberToAnalyze < mBoundsLower)
		  return FizzBuzzAnalysisResult.ErrorLowerBounds;
		if (numberToAnalyze > mBoundsUpper)

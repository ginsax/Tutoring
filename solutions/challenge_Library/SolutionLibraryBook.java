...
public class SolutionLibraryBook implements Comparable<SolutionLibraryBook> {
	 ...
	 * 
	 * @param isbn                  The unique international number assigned to this
	 *                              book.
	 ...
	 */
	public SolutionLibraryBook(final String isbn, final String title, final String author, final BookSeries series,
			final Genre genre, final Audience audience, final int publishingYear, final Fictionality fictionality,
			final int numberOfCopiesInStock, final int numberOfCopiesTotal) {
		mID.set(UUID.randomUUID());
	...
	public int compareTo(final SolutionLibraryBook otherBook) {
		...
		final int[] comparators = new int[] { comparisonAuthor, comparisonSeries, comparisonTitle, };
	...
	public String toString() {
		...
		return String.format("%s [%s] (%d)%s by %s. %s %s - %s. [%d/%d] copies in stock.", mTitle.get(), mISBN.get(),
				mPublishingYear.get(), seriesInformation, mAuthor.get(), mAudience.get().toString(),
				mGenre.get().toString(), mFictionality.get(), mNumberOfCopiesInStock.get(), mNumberOfCopiesTotal.get());
	}
}
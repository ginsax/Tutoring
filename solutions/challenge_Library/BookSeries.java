...
public class BookSeries implements Comparable<BookSeries> {
	 ...
	 * 
	 * @param seriesInformationToParse
	 *          String containing information about the series that needs to be
	 *          parsed.
	 * @throws InvalidBookSeriesNameException
	 */
	public BookSeries(final String seriesInformationToParse)
	    throws InvalidBookSeriesNameException {
		try {
			mSeriesName.set(seriesInformationToParse.substring(0,
			    seriesInformationToParse.lastIndexOf(" ")));
	...
	public int compareTo(final BookSeries otherBookSeries) {
		int comparisonSeriesName = mSeriesName.get()
		    .compareTo(otherBookSeries.seriesProperty().get());
		double comparisonPosition = mPositionInSeries
		    - otherBookSeries.mPositionInSeries;
		int comparisonTotal = mTotalNumberOfBooksInSeries
		    - otherBookSeries.mTotalNumberOfBooksInSeries;
		
		comparisonSeriesName = comparisonSeriesName < 0 ? -1
		    : comparisonSeriesName > 0 ? 1 : 0;
		comparisonPosition = comparisonPosition < 0 ? -1
		    : comparisonPosition > 0 ? 1 : 0;
	...
	public String getSeriesInformationString() {
		if (!isPartOfASeries()) { return ""; }
		
		String position;
		if ((mPositionInSeries % 1) == 0)
			position = "%.0f";
		else position = "%.1f";
		final String seriesInformation = String.format(
		    ", book " + position + "/%d of %s", mPositionInSeries,
		    mTotalNumberOfBooksInSeries, toString());

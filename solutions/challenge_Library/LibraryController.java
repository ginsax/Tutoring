...
public class LibraryController extends BorderPane {
	 ...
	 * 
	 * @param fileName
	 *          String file name that holds the book information.
	 * @throws EmptyFileNameException
	 *           The given {@code fileName} is empty.
	 */
	public LibraryController(final String fileName)
	    throws EmptyFileNameException {
		super();
		...
		final ObservableList<SolutionLibraryBook> bookList = loadBooksFromFile(
		    fileName);
		final FilteredList<SolutionLibraryBook> filteredBookList = createFilteredListFrom(
		    bookList);
		final SortedList<SolutionLibraryBook> sortedBookList = new SortedList<>(
		    filteredBookList, sortedBookComparator.get());
	 ...
	 * 
	 * @param bookList
	 *          A collection of LibraryBooks that are to be filtered.
	 * @return Returns a filtered list of library books.
	 */
	private FilteredList<SolutionLibraryBook> createFilteredListFrom(
	    final ObservableList<SolutionLibraryBook> bookList) {
		final FilteredList<SolutionLibraryBook> filteredBookList = new FilteredList<>(
		    bookList, p -> true);
		...
		predicateTitle.bind(Bindings.createObjectBinding(
		    () -> book -> book.getTitle().toLowerCase()
		        .contains(filterText_Title.getText().toLowerCase()),
		...
		predicateAuthor.bind(Bindings.createObjectBinding(
		    () -> book -> book.getAuthor().toLowerCase()
		        .contains(filterText_Author.getText().toLowerCase()),
		...
		predicateISBN.bind(Bindings.createObjectBinding(
		    () -> book -> book.getISBN().toLowerCase()
		        .contains(filterText_ISBN.getText().toLowerCase()),
		...
		predicateGenre.bind(Bindings.createObjectBinding(
		    () -> book -> book.getGenre()
		        ...
		        || (filter_Genre.getSelectionModel().getSelectedItem() == null),
		...
		predicateAudience.bind(Bindings.createObjectBinding(
		    () -> book -> book.getAudience()
		        .equals(filter_Audience.getSelectionModel().getSelectedItem())
		        || (filter_Audience.getSelectionModel().getSelectedItem() == null),
		...
		final ObjectBinding<Predicate<SolutionLibraryBook>> inStockBinding = Bindings
		    .createObjectBinding(
		        () -> predicateTitle.get().and(predicateAuthor.get())
		            .and(predicateYear.get()).and(predicateISBN.get())
		            .and(predicateGenre.get()).and(predicateAudience.get())
		            .and(predicateInStock.get()),
		        predicateTitle, predicateAuthor, predicateYear, predicateISBN,
		        predicateGenre, predicateAudience, predicateInStock);
		...
		final ObjectBinding<Predicate<SolutionLibraryBook>> defaultBinding = Bindings
		    .createObjectBinding(
		        () -> predicateTitle.get().and(predicateAuthor.get())
		            ...
		            .and(predicateGenre.get()).and(predicateAudience.get()),
		        predicateTitle, predicateAuthor, predicateYear, predicateISBN,
		        predicateGenre, predicateAudience);
	 ...
	 *         a file.
	 */
	private ObservableList<SolutionLibraryBook> loadBooksFromFile(
		...
		try {
			...
			mErrorAlert = new Alert(AlertType.ERROR, message, ButtonType.OK);
			mErrorAlert.show();
		}
		
		sortedBookComparator = new SimpleObjectProperty<>(
		    (book1, book2) -> book1.compareTo(book2));
	 ...
	 *          A sorted list of LibraryBooks to add to the table.
	 */
	private void setSortPrioritiesFor(

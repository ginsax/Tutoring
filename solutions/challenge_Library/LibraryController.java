...
public class LibraryController extends BorderPane {
	/**
	 * Alert that will be displayed if there is an issue with retrieving books from
	 * a file.
	 */
	...
	private FilteredList<SolutionLibraryBook> createFilteredListFrom(
		...
		final FilteredList<SolutionLibraryBook> filteredBookList = new FilteredList<>(bookList, p -> true);
		...
		predicateGenre.bind(Bindings.createObjectBinding(
				() -> book -> book.getGenre().equals(filter_Genre.getSelectionModel().getSelectedItem())
		...
		predicateAudience.bind(Bindings.createObjectBinding(
				() -> book -> book.getAudience().equals(filter_Audience.getSelectionModel().getSelectedItem())
		...
		predicateInStock.bind(Bindings.createObjectBinding(() -> book -> book.getNumberOfCopiesInStock() > 0,
		...
		final ObjectBinding<Predicate<SolutionLibraryBook>> inStockBinding = Bindings.createObjectBinding(
				() -> predicateTitle.get().and(predicateAuthor.get()).and(predicateYear.get()).and(predicateISBN.get())
						.and(predicateGenre.get()).and(predicateAudience.get()).and(predicateInStock.get()),
				predicateTitle, predicateAuthor, predicateYear, predicateISBN, predicateGenre, predicateAudience,
				predicateInStock);
	...
	/**
	 * Returns the alert dialog that is designed to be displayed in the event of any
	 * IOExceptions thrown.
	 * 
	 ...
	 * 
	 * @param fileName String file name that holds the book information.
	 * @return Returns a sorted list of LibraryBooks that have been retrieved from a
	 *         file.
	 */
	private ObservableList<SolutionLibraryBook> loadBooksFromFile(final String fileName) {
		...
		try {
			...
			final String message = String.format("Error loading books from file: %s", fileName);

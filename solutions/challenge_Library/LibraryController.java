...
public class LibraryController extends BorderPane {
  ...
  public LibraryController(final String fileName) throws EmptyFileNameException {
    ...
    if ((fileName == null) ||
        fileName.isEmpty()) { throw new EmptyFileNameException(); }
  ...
  private FilteredList<SolutionLibraryBook> createFilteredListFrom(final ObservableList<SolutionLibraryBook> bookList) {
    ...
    predicateYear.bind(Bindings.createObjectBinding(() -> book -> ("" +
                                                                   book.getPublishingYear())
                                                                       .contains(filterText_Year
                                                                           .getText()),
                                                    filterText_Year
                                                        .textProperty()));
    ...
    predicateGenre
        .bind(Bindings.createObjectBinding(() -> book -> book.getGenre()
    ...
    predicateAudience
        .bind(Bindings.createObjectBinding(() -> book -> book.getAudience()
    ...
    mCheckBoxInStock.selectedProperty().addListener((obs,
                                                     ov,
                                                     nv) -> {
      if (nv) {
  ...
  private ObservableList<SolutionLibraryBook> loadBooksFromFile(final String fileName) {
    ...
    sortedBookComparator = new SimpleObjectProperty<>((book1,
                                                       book2) -> book1
                                                           .compareTo(book2));

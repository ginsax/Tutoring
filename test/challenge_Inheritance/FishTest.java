...
public class FishTest {
	...
	public void test_CanMakeNoise() {
		...
		assertEquals(String.format("Testing that a fish can make a noise similar to '%s'.", expected), expected,
				actual);
	}
	...
	public void test_DefaultNumberOfFinsIs_MinimumInteger() {
		...
		assertEquals("Testing that the number of fins - if not specified - is equivalent to the minimum integer value.",
				expected, actual);
	}
	...
	public void test_NumberOfLegs_CannotBeSetToInvalidValues() {
		...
		for (int i = 3; i <= 100; i++) {
			...
			assertNotEquals(String.format("Testing that the number of fins cannot be set to %d.", expected), expected,
					actual);
		}
	}

...
public class MammalTest {
	...
	public void test_CanMakeNoise() {
		...
		assertEquals(String.format("Testing that a generic animal making noise sounds like '%s'.", expected), expected,
				actual);
	}
	...
	public void test_DefaultNumberOfLegsIs_MinimumInteger() {
		...
		assertEquals("Testing that the number of legs - if not specified - is equivalent to the minimum integer value.",
				expected, actual);
	}
	...
	public void test_NumberOfLegs_CanBeSet() {
		...
		assertEquals("Testing that the number of legs can be set to 0.", expected, actual);

		expected = 2;
		mammal = new Mammal(expected, dietType, skinType);
		actual = mammal.getNumberOfLegs();

		assertEquals("Testing that the number of legs can be set to 2.", expected, actual);
	}

	/**
	 * Testing that the number of legs cannot be set to values other than 0, 2, & 4.
	 */
	@Test
	public void test_NumberOfLegs_CannotBeSetToInvalidValues() {
		...
		assertNotEquals("Testing that the number of legs cannot be set to -1.", expected, actual);
		...
		assertNotEquals("Testing that the number of legs cannot be set to 3.", expected, actual);
		...
		for (int i = 5; i <= 100; i++) {
			...
			assertNotEquals(String.format("Testing that the number of legs cannot be set to %d.", expected), expected,
					actual);
		}
	}

...
public class LibraryControllerTest {
	 ...
	 *           if the given {@code fileName} is empty.
	...
	public void test_LibraryController_CanClearFilters()
	    throws EmptyFileNameException, IllegalArgumentException,
		...
		for (final Field field : LibraryController.class.getDeclaredFields()) {
			...
			if (field.getType().equals(TextField.class)
				...
				assertEquals("Tests that an unfiltered table has 6 items.", expected,
				    actual);
				...
				assertEquals("Tests that a filtered table has only 1 item.", expected,
				    actual);
				...
				actual = tableView.getItems().size();
				assertEquals(
				    "Tests that clearing the filters will restore the table to having all 8 items.",
				    expected, actual);
			}
		}
	}
	 ...
	 */
	@Test
	public void test_LibraryController_CanLoadFromFile()
	    throws EmptyFileNameException {
	...
	public void test_LibraryController_FilterTextBox_Author_FiltersCorrectly()
		...
		for (final Field field : fields) {
			...
			if (field.getType().equals(TextField.class)
				...
				expected = 2;
				actual = tableView.getItems().size();
				assertEquals("Tests that a filtered table has only 2 items.", expected,
				    actual);
	...
	public void test_LibraryController_FilterTextBox_Year_IsNotNull()
		...
		for (final Field field : fields) {
			...
			if (field.getType().equals(TextField.class)
				...
				assertNotNull(
				    "Tests that the year published filter text field of a library controller is not null",
				    textField);
			}
		}
	}
	...
	public void test_LibraryController_InStockCheckBox_IsNotNull()
		...
		for (final Field field : fields) {
			...
			if (field.getType().equals(CheckBox.class)) {
				...
				assertNotNull(
				    "Tests that checkbox of a library controller is not null.",
				    toggleButton);
			}
		}
	}
	...
	public void test_LibraryController_InStockCheckBoxFiltersCorrectly()
		...
		for (final Field field : fields) {
			...
			if (field.getType().equals(CheckBox.class)) {
				...
				expected = 5;
				actual = tableView.getItems().size();
				assertEquals("Tests that a filtered table has only 3 items.", expected,
				    actual);
				...
				actual = tableView.getItems().size();
				assertEquals(
				    "Tests that unselecting the filter checkbox will restore the table to having all 8 items.",
				    expected, actual);
			}
		}
	}

...
public class LibraryBook implements Comparable<LibraryBook> {
	...
	public LibraryBook(final String IBNS, final String tittle, final String auThor, final BookSeries Siries,
			final Genre genre, final Audience populationTarget, final int publishingYear,
			final Fictionality fictionality, final int stock, final int totalStock) {
		mID.set(UUID.randomUUID());
	...
	public int compareTo(final LibraryBook dummyBook) {
		...
		/*
		 * if (testah == 0) { return 0; } int actualTestah = testah/Math.abs(testah);
		 * return actualTestah;
		 */
		return testah;
	}
	...
	private String sortingTitle(final StringProperty actualTitle) {
		 ...
		 * 2).toLowerCase()=="the") { sortTitle=sortTitle.trim().substring(3).trim(); }
		 * else if (sortTitle.substring(0,1).toLowerCase() == "an") { sortTitle =
	...
	public String toString() {
		...
		final String returnString = getTitle() + " [" + getISBN() + "] (" + getPublishingYear() + ")"
				+ seriesProperty().get().getSeriesInformationString() + " by " + getAuthor() + ". " + getAudience()
				+ " " + getGenre() + " - " + getFictionality() + ". [" + getNumberOfCopiesInStock() + "/"
